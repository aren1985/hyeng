...
const SelectionPage = () => {
  ...
  const handleSelect = (type, _id, title) => {
    ...
    if (type === "lessons") {
      path = `/lessons4/lessvideo4?title=${encodeURIComponent(
        title
      )}&_id=${encodeURIComponent(_id)}`;
    } else if (type === "themes") {
      path = `/themes4/theme1?title=${encodeURIComponent(
        title
      )}&_id=${encodeURIComponent(_id)}`;
    } else if (type === "words") {
      path = `/words4/words1?title=${encodeURIComponent(
        title
      )}&_id=${encodeURIComponent(_id)}`;
    }
  ...
  return (
    <div className="flex flex-col items-center p-6">
      <h1 className="text-3xl font-bold text-purple-800 border-2 border-purple-800 mb-10 text-center py-2 px-6 rounded transform-gpu">
        Select Day
      </h1>
